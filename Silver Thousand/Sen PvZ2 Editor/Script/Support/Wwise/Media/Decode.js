                ...
                (function (Media) {
                    ...
                    (function (Decode) {
                        function process(source, destination) {
                            ...
                            Script.assert(operating_system === "Windows" || operating_system === "macOS" || operating_system === "Linux", Sen.Kernel.Language.get("wwise.media.decode.unsupported_operating_system"));
                            const vgmstream_program_third_path = Script.Home.query("~/../thirdapp/vgmstream");
                            const vgmstream_program_file_path = `${vgmstream_program_third_path}/vgmstream-cli${(operating_system === "Windows" ? ".exe" : "")}`;
                            Script.assert(Sen.Kernel.FileSystem.is_file(vgmstream_program_file_path), Sen.Kernel.Language.get("wwise.media.decode.cannot_find_third"));
                            const command = `${vgmstream_program_file_path} -o "${destination}" "${source}"`;
                            const program_result = Sen.Kernel.Process.execute(command);
                            let regex_result = /^encoding: (.+)$/m.exec(program_result);
                            Script.assert(regex_result !== null);
                            ...
                            Script.assert(format !== null, Script.format(`${Sen.Kernel.Language.get("wwise.media.decode.unknown_audio_format")}`, encoding));
                            Script.Console.display(`${Sen.Kernel.Language.get("wwise.media.audio_format")}:`, `${encoding} (${format})`, Script.Definition.Console.Color.GREEN);
                            return;
                        }
