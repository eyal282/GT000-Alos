                        ...
                        (function (Decode) {
                            ...
                            function forward() {
                                Sen.Script.Executor.push_as_module({
                                    ...
                                    configuration_file: Script.Home.query("~/Executor/Configuration/popcap.ptx.decode.json"),
                                    configuration: undefined,
                                    direct_forward(argument) {
                                        Executor.is_valid_source(argument, false);
                                        Script.Console.obtained(argument.source);
                                        Executor.defined_or_default(argument, "destination", `${Sen.Kernel.Path.except_extension(argument.source)}.png`);
                                        Script.Console.output(argument.destination);
                                        Executor.load_bigint(argument, "format", this.configuration, Detail.format(), Sen.Kernel.Language.get("popcap.ptx.decode.format"));
                                        argument.size = {};
                                        Executor.input_range(argument.size, "width", this.configuration.size, [1n, 16384n], Sen.Kernel.Language.get("popcap.ptx.decode.width"));
                                        if (argument.format === "rgb_pvrtc4_a_8") {
                                            this.configuration.size.height = argument.size.width;
                                        }
                                        Executor.input_range(argument.size, "height", this.configuration.size, [1n, 16384n], Sen.Kernel.Language.get("popcap.ptx.decode.height"));
