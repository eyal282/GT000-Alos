...
(function (Sen) {
        ...
        (function (Support) {
            ...
            (function (Wwise) {
                ...
                (function (Media) {
                    var Encode;
                    (function (Encode) {
                        Encode.k_sample_conversion_settings = `<?xml version="1.0" encoding="utf-8"?>
    ...
    `;
                        function cast_wwise_internal_path(raw) {
                            const operating_system = Sen.Kernel.OperatingSystem.current();
                            if (operating_system === "Windows") {
                                return raw;
                            }
                            if (operating_system === "macOS") {
                                Script.assert(raw.startsWith("/"), Sen.Kernel.Language.get("wwise.media.encode.path_must_starts_with_seperator"));
                                return `Z:${raw}`;
                            }
                            Script.assert(false, Sen.Kernel.Language.get("wwise.media.encode.unsupported_operating_system"));
                        }
                        Encode.cast_wwise_internal_path = cast_wwise_internal_path;
                        function process(source, destination, format) {
                            const operating_system = Sen.Kernel.OperatingSystem.current();
                            Script.assert(operating_system === "Windows" || operating_system === "macOS", Sen.Kernel.Language.get("wwise.media.encode.unsupported_operating_system"));
                            const wwise_program_file_path = Script.Support.Wwise.Media.Common.search_path("WwiseConsole");
                            Script.assert(wwise_program_file_path !== null, Sen.Kernel.Language.get("wwise.media.encode.could_not_find_wwise_console"));
                            ...
                            while (true) {
                                Sen.Kernel.Process.execute(wwise_create_new_project_command);
                                if (Sen.Kernel.FileSystem.is_file(wwise_wproj_file)) {
                                    break;
                                }
                            }
                            ...
                            Sen.Kernel.FileSystem.write_file(wwise_wsources_file, Sen.Kernel.XML.serialize(wwise_wsources));
                            Sen.Kernel.FileSystem.write_file(`${wwise_project_dir}/Conversion Settings/Sample Conversion Settings.wwu`, Encode.k_sample_conversion_settings);
                            if (Sen.Kernel.FileSystem.is_file(`${wwise_project_dir}/Sample.wav`))
                                Sen.Kernel.FileSystem.Operation.remove(`${wwise_project_dir}/Sample.wav`);
                            Sen.Kernel.FileSystem.Operation.copy(source, `${wwise_project_dir}/Sample.wav`);
                            const platform = {
                                pcm: "Android",
                                adpcm: "Android",
                                vorbis: "Android",
                            ...
                            const process_result = Sen.Kernel.Process.execute(wwise_command);
                            const result_string_list = process_result.split("\n");
                            Script.assert(result_string_list[result_string_list.length - 2] === "Process completed successfully.", Sen.Kernel.Language.get("wwise.media.encode.failed"));
                            Sen.Kernel.FileSystem.Operation.remove(destination);
                            Sen.Kernel.FileSystem.Operation.copy(`${wwise_project_dir}/GeneratedSoundBanks/${platform}/Sample.wem`, destination);
                            Sen.Kernel.FileSystem.Operation.remove_all(wwise_project_dir);
                            return;
                        }
                        Encode.process = process;
                        function process_fs(source, destination, format) {
                            process(source, destination, format);
                            return;
                        }
                        Encode.process_fs = process_fs;
