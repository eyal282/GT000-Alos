...
(function (Sen) {
    ...
    (function (Script) {
        ...
        (function (Support) {
            var PopCap;
            (function (PopCap) {
                var ReflectionObjectNotation;
                (function (ReflectionObjectNotation) {
                    var DecodeByLooseConstraints;
                    (function (DecodeByLooseConstraints) {
                        let writer = undefined;
                        let json_destination;
                        const json_table = {
                            end_bytecode: 0xffn,
                        ...
                        let str_length_2 = 0n;
                        function write() {
                            Sen.Kernel.FileSystem.write_file(json_destination, writer.toString());
                            return;
                        }
                        DecodeByLooseConstraints.write = write;
                        function get_id(stream) {
                            const uid = stream.readUint32().toString(16);
                            return "0".repeat(8 - uid.length) + uid;
                        }
                        DecodeByLooseConstraints.get_id = get_id;
                        function read_utf8_character(stream, size) {
                            let value = 0n;
                            ...
                            if (current < 128n) {
                                ...
                                throw new Error(`${Sen.Kernel.Language.get("popcap.rton.force_decode.position")}: ${stream.read_position} : ${Sen.Kernel.Language.get("popcap.rton.force_decode.invalid_utf8_first_character")}`);
                            }
                            ...
                            while (extra_size > 0) {
                                ...
                                if ((current & 192n) != 128n) {
                                    throw new Error(`${Sen.Kernel.Language.get("popcap.rton.force_decode.position")}: ${stream.read_position} : ${Sen.Kernel.Language.get("popcap.rton.force_decode.invalid_utf8_extra_character")}`);
                                }
                                value = (value << 6n) | (current & 63n);
                            }
                            return value;
                        }
                        DecodeByLooseConstraints.read_utf8_character = read_utf8_character;
                        function read_utf8_string(stream) {
                            let str = "";
                            ...
                            if (size !== actual_size) {
                                Script.Console.warning(`${Sen.Kernel.Language.get("popcap.rton.force_decode.position")}: ${stream.read_position}: ${Sen.Kernel.Language.get("popcap.rton.force_decode.invalid_string_size")}: ${Sen.Kernel.Language.get("got")} ${size} ${Sen.Kernel.Language.get("popcap.rton.force_decode.but_actual")} ${actual_size}`);
                            }
                            return str;
                        }
                        DecodeByLooseConstraints.read_utf8_string = read_utf8_string;
                        function read_rtid(stream) {
                        ...
                        DecodeByLooseConstraints.read_rtid = read_rtid;
                        function read_binary(stream) {
                            stream.read_position += 1n;
                            return `BINARY(${stream.readStringByVarInt32()}, ${stream.readVarInt32()})`;
                        }
                        DecodeByLooseConstraints.read_binary = read_binary;
                        function read_array(stream) {
                        ...
                        DecodeByLooseConstraints.read_array = read_array;
                        function catch_error(bytecode, stream, function_name) {
                            write();
                            throw new Error(`${Sen.Kernel.Language.get("popcap.rton.force_decode.invalid_bytecode")}: ${bytecode.toString(16)} | ${Sen.Kernel.Language.get("popcap.rton.force_decode.position")}: ${stream.read_position} ${Sen.Kernel.Language.get("in")} ${function_name}`);
                        }
                        DecodeByLooseConstraints.catch_error = catch_error;
                        function read_bytecode(stream, bytecode) {
                            switch (bytecode) {
                                case 0x0n: {
                        ...
                        DecodeByLooseConstraints.read_bytecode = read_bytecode;
                        function read_bytecode_property(stream, bytecode) {
                            switch (bytecode) {
                        ...
                        DecodeByLooseConstraints.read_bytecode_property = read_bytecode_property;
                        ...
                        DecodeByLooseConstraints.read_object = read_object;
                        function process(stream, destination) {
                        ...
                        DecodeByLooseConstraints.process = process;
                        function process_fs(source, destination) {
                            if (writer === undefined) {
                                writer = new Sen.Kernel.JsonWriter();
                            }
                            process(new Sen.Kernel.DataStreamView(source), destination);
                            return;
                        }
                        DecodeByLooseConstraints.process_fs = process_fs;
                    })(DecodeByLooseConstraints = ReflectionObjectNotation.DecodeByLooseConstraints || (ReflectionObjectNotation.DecodeByLooseConstraints = {}));
                })(ReflectionObjectNotation = PopCap.ReflectionObjectNotation || (PopCap.ReflectionObjectNotation = {}));
